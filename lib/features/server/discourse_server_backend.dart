...
class DiscourseServerBackend extends AsyncNotifier<DiscourseServerModel> {
  ...
  static var categoryProvider = FutureProvider<List<DiscourseCategory>>(
    (ref) {
      var cds = ref.watch(DiscourseServer.provider.notifier);
      return cds.getCategories();
    },
  );

  Future<DiscourseServerInfo> getServerInfo() async {
    if (!state.hasValue) {
      return Future.error(NoServerException, StackTrace.current);
    }

    if (state.hasError) {
      return Future.error(state.error!, StackTrace.current);
    }
  ...
  FutureOr<DiscourseServerModel> build() async {
    var currentAccount = await ref.watch(Account.provider.future);
    if (currentAccount.server.value == null) {
      state = AsyncValue.error(NoServerException, StackTrace.current);
  ...
  Future<DiscourseTopicModel> getTopic(
      {required int topicId,
      required String apiKey,
      required DiscourseCategory parentCategory}) async {
    if (!state.hasValue) {
  ...
  String getImgUrlFromTemplate(String template, {int size = 32}) {
    if (!state.hasValue) {
      throw NoServerException;
    }
  ...
  Future<List<DiscourseCategory>> getCategories() async {
    if (!state.hasValue) {
      return Future.error(NoServerException);
    }
    ...
    try {
      ...
      var allCats = await getServerInfo();
